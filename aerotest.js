document.addEventListener('DOMContentLoaded', function () {
    ...
    function managePlaceholder(inputId, placeholderText, isClear) {
        const inputElement = document.getElementById(inputId);
        if (isClear && inputElement.value === '') {
            inputElement.placeholder = '0' + placeholderText;
            inputElement.value = '0';
        } else if (!isClear && inputElement.value == '0') {
            inputElement.placeholder = placeholderText;
            inputElement.value = '';
        }
    }

    function parseTimeToSeconds(timeStr, isTest = false) {
    ...
    function calculateWindYawAngle(bikeSpeedMS, windSpeedMS, windAngleDegrees) {
        ...
        return Math.atan2(windSpeedMS * Math.sin(windAngleRadians), bikeSpeedMS + windSpeedMS * Math.cos(windAngleRadians));
    }
    ...
    function calculateCdA() {
        ...
        for (let test of tests) {
            if (!isNaN(test.distance) && test.time && !isNaN(test.power)) {
                const timeSeconds = parseTimeToSeconds(test.time, true);
        ...
        if (cdaValues.length > 0) {
            ...
            const averageSpeed = cdaValues.reduce((total, cda, index) => total + (tests[index].distance * 1000 / parseTimeToSeconds(tests[index].time, true)) * 3.6, 0) / cdaValues.length;
